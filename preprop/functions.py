...
def nilearn_denoise(in_file, brain_mask, wm_csf_mask,
    ...
    wm_csf_nii = nb.Nifti1Image(nb.load(wm_csf_mask).get_data(), 
                                np.around(nb.load(wm_csf_mask).get_affine(), 2), 
                                nb.load(wm_csf_mask).get_header())
    time_nii = nb.Nifti1Image(nb.load(in_file).get_data(),
                              np.around(nb.load(in_file).get_affine(), 2), 
                              nb.load(in_file).get_header())
    
    # infer shape of confound array # not ideal
    ...
    wm_csf_regressor = high_variance_confounds(time_nii, 
                                               mask_img=wm_csf_nii, 
                                               detrend=True)

    # create Nifti Masker for denoising
    denoiser = NiftiMasker(mask_img=brain_mask, 
                           standardize=True, 
                           detrend=True, 
                           high_pass=bandpass[1], 
                           low_pass=bandpass[0], t_r=tr)
